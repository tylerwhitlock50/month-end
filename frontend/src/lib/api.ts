  ...
  if (envUrl && envUrl.trim().length > 0) {
    if (typeof window !== 'undefined') {
      try {
        ...
        const envHostIsLocal = ['localhost', '127.0.0.1', '0.0.0.0', 'backend'].includes(parsed.hostname)
        const browserHostIsLocal = ['localhost', '127.0.0.1', '0.0.0.0', 'backend'].includes(window.location.hostname)

        if (envHostIsLocal && !browserHostIsLocal) {
          const port = import.meta.env.VITE_API_PORT || parsed.port || '8000'
          const portSegment = port ? `:${port}` : ''
          const protocol = window.location.protocol || parsed.protocol
          return `${protocol}//${window.location.hostname}${portSegment}`
        }
      } catch (_error) {
        // If env URL is not parseable, fall through to using it as-is
      }
    }
